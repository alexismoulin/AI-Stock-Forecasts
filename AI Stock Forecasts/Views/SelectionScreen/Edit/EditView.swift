...
struct EditView: View {
    ...
    init(company: CustomCompany) {
        ...
        _sector = State(
            initialValue: Sector.allCases.first(where: { $0.rawValue == company.wrappedSector })
                ?? Sector.all
        )
    }
    ...
    var body: some View {
        Form {
            ...
            Section(header: Text("Name")) {
                modificationMode ?
                    AnyView(TextField(company.wrappedName, text: $name))
                    : AnyView(Text(company.wrappedName))
            }
            // Text in display mode or Texfield in modification mode
            Section(header: Text("Stock Symbol")) {
                modificationMode ?
                    AnyView(TextField(company.wrappedId, text: $id))
                    : AnyView(Text(company.wrappedId))
            }
            // Text in display mode or Texfield in modification mode
            Section(header: Text("Twitter @")) {
                modificationMode ?
                    AnyView(TextField(company.wrappedArobase, text: $arobase))
                    : AnyView(Text(company.wrappedArobase))
            }
